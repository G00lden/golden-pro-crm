...
if (userVersion !== 11002) throw new Error(`Expected schema 11002, got ${userVersion}`);
for (const required of [
  "request_number",
  "client_request_id",
  "status",
...
for (const required of [
  ...
  if (!columns("delivery_review_requests").has(required)) {
    throw new Error(`delivery_review_requests.${required} is missing`);
  }
}
...
const deliveryReviewMigration = db.prepare("SELECT release FROM schema_migrations WHERE version = 10900").get() as { release?: string };
