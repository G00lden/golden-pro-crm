...
| ID | Risk | Closed | Proof |
|----|------|--------|-------|
| R-008 | WhatsApp inbound webhook accepted forged unsigned POSTs | 2026-06-24 | Added HMAC (`x-hub-signature-256` keyed by `WHATSAPP_APP_SECRET`) + shared-secret fallback, fail-closed (503) in prod. `verifyWhatsAppWebhookAuth` unit-tested 7/7; wired into both POST routes in `routes-whatsapp.ts`. |
| R-009 | `POST /api/whatsapp/send-template` lacked `requireAdmin` | 2026-06-24 | Added `requireAdmin` middleware. Live re-probe: non-admin → 403 (was 500/reachable). |
| R-010 | Tracking pixels fired with no per-visitor consent | 2026-06-24 | `src/consent.ts` + `ConsentBanner.tsx`; GTM/GA4/Meta init gated on `granted` consent in `main.tsx`. Banner renders on public pages only. |
