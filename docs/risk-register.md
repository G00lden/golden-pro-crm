...
| R-008 | **WhatsApp inbound webhook accepts forged unsigned POSTs.** Live-confirmed 2026-06-24: `POST /webhooks/whatsapp` with no signature → 200. Forged inbound can inject fake customer confirmations / reschedules / technician acks into SQLite. | High | Medium | Add `x-hub-signature-256` HMAC verification (mirror `storeWebhook.ts` / `salla.ts`), mandatory in prod. `server/whatsappWebhook.ts` + `server/routes-whatsapp.ts`. | codex | 2026-06-24 | Open |
| R-009 | **`POST /api/whatsapp/send-template` lacks `requireAdmin`.** Live-confirmed 2026-06-24: non-admin token reaches the handler (500, not 403). Any authenticated user can trigger outbound WA (still gated by `OUTBOUND_LAUNCH_CODE`). | Medium | Medium | Add `requireAdmin` middleware to `send-template` (and review `/messages`, `/stats`, `/devices`). `server/routes-whatsapp.ts`. | codex | 2026-06-24 | Open |
| R-010 | **Tracking pixels (GTM/GA4/Meta) fire at page load with no per-visitor consent.** `src/main.tsx` inits all three behind a deploy-time flag only. Violates PDPL + Meta ad-policy once ads run. | Medium | High | Build a cookie-consent banner; gate init behind granted consent (checklist 5.7). | claude | 2026-06-24 | Open |
