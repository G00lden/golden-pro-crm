...
| 1.8 | Backup + restore for SQLite + Supabase documented | ✗ | claude | **found real unmerged work**: branch `codex/p0-security-backup-salla` (commit `e3853dc`, 2026-06-19) has a working `docs/backup-restore.md`, `scripts/backup-now.mjs` (zip staging + checksum, `.runtime/backups/`), `scripts/restore-latest-backup.mjs`, `backup-now.cmd` / `restore-last-good.cmd` — inspected the script, it's functional (not a stub), not a rename/duplicate of anything already on `main`. Branch is 44 commits behind `main` (its other commit, ZATCA QR, was superseded by the version that shipped via `913e87e`/`8ebd237`). **Action needed**: cherry-pick the backup/restore commit onto a fresh branch off current `main`, verify `npm run backup:now` / restore round-trip, then this can move to ◐/✓. Not done by Supervisor directly since this touches data-recovery paths and is squarely `claude`'s owned lane. |
