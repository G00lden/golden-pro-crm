...
## إصدار 1.3.3: الربط ثنائي الاتجاه

أصبح مسار الطلبات يعتمد القواعد التالية:

- مزامنة الطلبات تقرأ جميع الصفحات المعلنة بالتتابع، 30 طلبًا لكل صفحة وحتى 200 صفحة، وتفشل بوضوح إذا تغيّر العدد أثناء اللقطة أو ظهرت صفحة ناقصة أو رقم طلب مكرر.
- الاستيراد التاريخي يستخدم إسقاطًا آمنًا: يحدّث العميل والمنتج والطلب، لكنه لا ينشئ تركيبات أو حجوزات أو تذكيرات أو إشعارات جماعية.
- `order.created` اللحظي فقط يمكنه تشغيل مسار الطلب التشغيلي، وإذا لم يوجد رقم جوال يتحول تلقائيًا إلى الإسقاط الآمن.
- كل حدث طلب يدخل أولًا إلى `salla_order_inbox` بمعرّف حتمي ومنع تكرار، ثم تُقرأ تفاصيل الطلب الرسمية من Salla API.
- تشمل أحداث الطلب المدعومة أحداث العميل داخل الطلب، الكوبون، الدفع، البنود، السعر، العنوان، الإنشاء والتحديث والحالة والإلغاء والاسترجاع والحذف، ودورة الشحن وشحنة الاسترجاع كاملة.
- كل تعديل صادر من النظام يسجل في `salla_order_commands`، ويرسل مرة واحدة، ثم يقرأ الحالة النهائية من سلة قبل تسجيل النجاح.
- إذا ضاعت نتيجة طلب الكتابة، لا يعاد الإرسال تلقائيًا؛ تُجرى مصالحة قراءة أولًا لمنع تكرار الرسائل أو آثار الدفع والشحن.
- إذا تعذر إثبات النتيجة بعد المصالحة يبقى الأمر في `outcome_unknown` ولا يسمح الطلب المطابق بإرسال الكتابة مرة ثانية؛ يلزم أولًا أن تثبت القراءة المرجعية النتيجة.
- تستخدم سجلات الأحداث والأوامر رمز مهلة ذريًا في SQLite وSupabase ومعاملة في Firestore، فلا يستطيع عامل انتهت مهلته الكتابة فوق العامل الذي استلمها بعده.
- أحداث الطلب المختلفة تُرتّب لكل طلب داخل عملية الخادم، وتستخدم كتابة الطلب مقارنة ذرية بوقت `remote_updated_at` لرفض اللقطة الأقدم حتى عند تنافس أكثر من عامل.
- يرفض حدث `app.store.authorize` إذا كان معرّف التاجر مختلفًا عن المتجر المرتبط، فلا تستبدل رموز متجر آخر الاتصال الحالي.
- الحالة البعيدة ووقت آخر تحديث ومصدر المزامنة محفوظة منفصلة عن حالة رحلة العمل المحلية وتصنيف البنود والروابط والجدولة.

المسارات الجديدة:

- `GET /api/integrations/salla/order-statuses`
- `POST /api/integrations/salla/orders/:id/status`
- `PUT /api/integrations/salla/orders/:id`
- `GET /api/store/orders/events` لبث تحديثات الطلبات اللحظية عبر SSE مصادق.

الحالات القياسية ترسل عبر `slug`، أما الحالة الفرعية المخصصة فترسل عبر `status_id` بعد تمييزها من بيانات `type/original/parent` القادمة من سلة. تعرض الواجهة خيار `restore_items` عند انتقال حالة الاستعادة.

الحقول التي يمكن لمسار `PUT` إرسالها هي فقط المسموحة رسميًا: العميل الضيف، المستلم، الشحن والعنوان الوطني، الدفع المعلق، الكوبون، والموظفون. تعرض الواجهة هذه المجموعات كلها. معرّفا الدولة والمدينة في العنوان الوطني رقمان من سلة وليسا اسمين نصيين. يطبق الخادم قيود حالة الطلب حتى لو تجاوز العميل تحقق الواجهة.

### ملكية الحقول والآثار الجانبية

- سلة هي المرجع لحالة التجارة والدفع والإلغاء والاسترجاع وبنود الطلب وأسعارها.
- البرنامج هو المرجع للتصنيف اليدوي ورحلة الخدمة والفني والتركيب والحجز والملاحظات والجدولة الداخلية.
- تغيير حالة سلة قد يشغّل رسائل العميل أو إجراءات الدفع والشحن؛ لذلك تتطلب الواجهة تأكيدًا صريحًا.
- البث اللحظي الحالي داخل عملية الخادم ويعمل مع عملية VPS واحدة. عند الانتقال إلى عدة عمليات أو خوادم يجب إضافة Redis أو Postgres Pub/Sub قبل التوسع.
