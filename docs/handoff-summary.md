...
## تحديث 2026-07-12 — الإصدار 1.3.2 «مزامنة العملاء الكاملة» [Codex]

- أثبت فحص الإنتاج أن سلة تعلن 4610 عميلًا، بينما حد الثلاث صفحات كان يسحب 90 فقط، وصفحة CRM كانت تعرض حدًا أقصى 250 كأنه الإجمالي.
- فصلت صفحات العملاء إلى 60 سجلًا وحتى 200 صفحة، وأصبح النجاح مشروطًا بإكمال `totalPages`؛ فرق `total` الصغير بعد اكتمال الصفحات يحفظ كتحذير ظاهر، بينما الفرق الكبير أو الهوية المتكررة يبقيان فشلًا قابلًا للإعادة، واعتمد الإدخال معرف عميل سلة بدل الهاتف.
- أضيف عد حقيقي وتصفح وبحث شامل للعملاء، مع تحسين حساب إحصاءات العناية ودعم جلب Supabase على دفعات.
- صُحح إعداد المالك الذي كان يشير إلى `local-dev-owner`. تفاصيل الترحيل والآثار الجانبية موثقة في `docs/salla-customer-sync-1.3.2-ar.md`.
- نتيجة الإنتاج بعد الاستيراد: `4767` عميلًا (`4598` بمعرف سلة فريد + `52` متجر تاريخي + `117` يدوي)، وصفر تكرار في `store_customer_id`. تُركت السجلات التاريخية لأنها تشمل مراجع طلبات ولا يجوز دمجها بالهاتف وحده.
- التحقق قبل النشر: `121/121` اختبار وحدة، وlint/build، وتدقيق مصدر بلا تحذيرات، وفحص Bash وPowerShell ناجح.

## تحديث 2026-07-12 — الإصدار 1.3.1 «ربط سلة المستقر» [Codex]

- أصبح تجديد رمز سلة متسلسلاً لكل مالك، دون إعادة تلقائية لطلب التجديد الأحادي، مع إعادة محاولة محدودة وآمنة لطلبات القراءة فقط.
- فُصل خطأ المزامنة المؤقت عن حالة الاتصال، وأصبحت حالة JSON تكتب ذريًا بصلاحية `0600` ولا تستبدل صامتًا عند التلف.
- صُححت قراءة `data.merchant` و`/store/info`، وأضيف رابط HTTPS فعلي لفتح المتجر مع حالات وصول واضحة.
- حُدّد التصفح إلى 30 نتيجة واعتمد `pagination.totalPages`، وصُحح المجدول ليستخدم مفتاح المالك الموثوق.
- أصبحت نسخ VPS خارج build context وتحفظ حالة سلة مع checksum وقفل تزامن، وأصبح الاسترجاع fail-closed مع فحص SQLite والملكية والصلاحيات قبل التشغيل.
- لا توجد ترقية قاعدة بيانات. الآثار الجانبية والاختبارات موثقة في `docs/salla-release-1.3.1-ar.md`.
- التحقق قبل النشر: `99/99` اختبارات وحدة، وlint/build، وفحص Bash وPowerShell ناجحة.
- أصبح `BUILD_COMMIT` في Docker Compose يأخذ قيمة حزمة النشر صراحةً، حتى لا تعرض `/api/health` رقم بناء قديمًا محفوظًا في ملف البيئة.

## تحديث 2026-07-12 — الإصدار 1.1.0 «فاتورة منضبطة» [Codex]

- وُحّد حساب خصم وضريبة البنود مع ترويسة الفاتورة بتوزيع دقيق للهللات، وأضيف حفظ نوع الخصم وقيمته ونوع الفاتورة B2C/B2B.
- صُحح التصنيف الذي كان يعتمد على مبلغ 1000 ريال وحده؛ فاتورة المستهلك النهائي تبقى مبسطة مهما بلغت، والاختيار الصريح متاح في النموذج.
- وُحد QR بين العميل والخادم مع تحقق رقم VAT وطول UTF-8، وأصبحت الواجهة تصف الدعم بدقة كحقول المرحلة الأولى دون ادعاء تكامل المرحلة الثانية.
- صُححت معاينة A4 والجدول على الجوال، وعُزلت قواعد الجوال عن ملف PDF، وأصبح تقسيم الصفحات يحافظ على الصفوف والأقسام كاملة.
- أضيفت ترقية SQLite `10100` وترحيل Supabase `20260712011000_invoice_tax_integrity.sql`؛ التغيير إضافي ولا يعيد كتابة الفواتير التاريخية تلقائيًا.
- التحقق: 45/45 وحدة، lint/build، Smoke 15/15، Golden Path 10/10، تدقيق مصدر بلا تحذيرات، صفر ثغرات runtime، وPDF فعلي من 24 بندًا في 3 صفحات دون صفوف مقطوعة.
- التفاصيل والآثار الجانبية وحدود الامتثال: `docs/invoice-tax-integrity-1.1.0.md`.
...
## تحديث 2026-07-12 — الإصدار 1.0.5 «زمن صحيح» [Codex]

- أضيف `shared/date.ts` لحساب أشهر التقويم دون overflow وبصيغة ISO مستقلة عن timezone.
- استبدلت الحسابات المتكررة في دورة الصيانة والحجوزات وstore webhook والواجهة بالدالة المشتركة.
- نهاية الشهر تُقيد إلى آخر يوم صالح: `2026-01-31 + 1 = 2026-02-28`، والسنة الكبيسة تعطي `2024-02-29`.
- التواريخ المستحيلة أو غير ISO ترفض صراحة؛ مسار الحجز الذي كان يقبل fallback ما زال يستخدم fallback واضحًا إلى تاريخ اليوم.
- الأثر الجانبي: جداول الصيانة المستقبلية المنشأة أو المكتملة بعد الإصدار قد تختلف عن ناتج overflow القديم؛ لا تعدّل المواعيد التاريخية المخزنة تلقائيًا.
- التحقق: اختبارات التاريخ 5/5، ولا توجد استدعاءات `setMonth`/`setUTCMonth` متبقية في `server/` أو `src/`، وlint ناجح.

## تحديث 2026-07-12 — الإصدار 1.0.6 «حدود آمنة» [Codex]

- أضيفت مخططات Zod لعمليات CRUD الأساسية: العملاء، المنتجات، التركيبات، الفنيون، الحجوزات، العروض، الفواتير والإعدادات.
- تتحقق المدخلات من الأطوال، الأرقام، النسب، الأموال، التواريخ الموجودة فعليًا، الوقت `HH:MM`، الحالات، وحدود arrays قبل منطق الأعمال أو قاعدة البيانات.
- تتحقق معاملات `:id` من حد أقصى 160 حرفًا، وتزال الحقول المجهولة افتراضيًا؛ لذلك لا يمكن تمرير حقل صلاحية مثل `role` عبر endpoint عميل.
- خصم النسبة فوق 100% والتواريخ المستحيلة مثل `2026-02-30` تعود بخطأ `400` منظم مع path ورسالة.
- الأثر الجانبي: payloads القديمة غير المطابقة ستحتاج تصحيحًا؛ الحقول غير المعرّفة في العقد لن تحفظ بعد الآن.
- التحقق: اختبارات validation 4/4 وGolden Path 9/9 على قاعدة جديدة متضمنًا رفض payloads الخاطئة وإزالة حقل privilege، وlint ناجح.

## تحديث 2026-07-12 — الإصدار 1.0.7 «هوية مستقرة» [Codex]

- أزيلت ترقية أول مستخدم Firebase إلى admin تلقائيًا؛ الإدارة تتطلب UID داخل `ADMIN_UIDS` أو بريدًا موثقًا داخل `BOOTSTRAP_ADMIN_EMAILS` أو دعوة مسبقة.
- حسابات local test تحفظ `email=NULL` بدل البريد المشترك الذي كان يسبب UNIQUE conflict، والمالك المحلي المصرح وحده admin بينما بقية UIDs مستخدمون عاديون.
- ترقية المخطط `10007` تمسح البريد الاصطناعي القديم من سجلات `local-dev` بعد إنشاء backup إنتاجي، دون حذف الحساب أو دوره.
- ربط الدعوات بالبريد أصبح case-insensitive، والبريد يحفظ lowercase، وتعارض البريد في update يعود `409` بدل خطأ قاعدة عام.
- أضيف Zod لقائمة وإنشاء وتحديث ومعاملات إدارة المستخدمين، مع allowlist للأدوار وأسماء الصلاحيات.
...
## تحديث 2026-07-12 — الإصدار 1.0.9 «النواة المرتبة» [Codex]

- أصبح CRUD المقيّد بالمالك وفحص المراجع خلف عقدي `OwnedRepository` و`FirestoreLikeStore` مستقلين عن SQLite وSupabase وFirestore.
- انتقلت سياسة اختيار مزود بيانات الواجهة إلى `src/dataProvider.ts` بدل بقائها مدفونة داخل ملف API الكبير.
- أضيفت اختبارات عقد تثبت عزل المستأجرين، ومنع تعديل حقول الملكية، وفحص المراجع والحذف ضمن المالك الصحيح.
- وُثّق اتجاه الاعتمادات وحدود الآثار الجانبية وقرار استخدام التركيب والعقود بدل وراثة مصطنعة في `docs/architecture.md`.
- أضيف `server/bootstrap.ts` ليقرأ `ENV_FILE` قبل تقييم وحدات قاعدة البيانات؛ الاختبار التكاملي يشغّل حزمة الإنتاج من ملف env مؤقت ويتحقق أن مسارات Vite والمصدر تعود 404.
- الأثر الجانبي: لا تغيير في HTTP API أو مخطط البيانات؛ الإضافة الداخلية تفرض تمرير UID صريح لكل عملية عامة وتجعل إضافة adapter جديد مرتبطة بالعقد.
- التحقق: unit 39/39، smoke 15/15، Golden Path 10/10، lint/build، وفحصا الأمن ناجحة.

## مراجعة Supervisor — 2026-07-12 — PR #63 متوقف عند البوابة

1. **ما دُمج/نُشر:** لم يُدمج أو يُنشر شيء على الإنتاج. اكتملت الإصدارات
   1.0.0–1.0.9 على `codex/v1-remediation` وفُتح PR #63 كمسودة.
2. **المرفوض/المتوقف:** يمنع النظام Codex من اعتماد عمله؛ يلزم مرور Claude أو
   Hermes. GitHub رفض رفع `ci.yml` و`deploy.yml` لأن PAT والتطبيق بلا صلاحية
   `workflow`. كذلك يلزم إثبات دخول Firebase حقيقي وبقاء حساب المالك admin.
3. **فرق قائمة الإصدار:** لا عناصر قُلبت إلى مكتمل؛ الحالة 26/78، وبقيت 5
   بوابات أمنية صلبة.
4. **قائمة الإجراء التالي:** مراجعة مستقلة حسب
   `docs/tasks/claude-review-v1-remediation-2026-07-12.md`، منح صلاحية workflow،
   تحديد UID/بريد المسؤول، ثم staging وفحص المسارات العامة قبل الدمج.
5. **المخاطر/التصعيد:** النطاق الحي ما زال Vite dev، يعرض ملفات المصدر ونسخة
   package 0.0.0 ولا يرسل HSTS. نشر المرشح قبل تحقق الهوية قد يقفل دخول المالك.
...
## آخر تحديث: 2026-07-15 — منصة تشغيل الجوال BreeXe 2.1.0 [Codex]

- أضيفت واجهات `/api/mobile/v1` للأجهزة، نبضات الصحة، دفعات الأحداث، أوامر CRM، السياسة، لوحة الموظف وذاكرة تعريف المتصل.
- أضيف مركز «الجوال والموظفون» في CRM لإسناد الموظف والفرع وشريحة العمل، إرسال طلب اتصال، إدارة سياسة واتساب وتجربة رقم لمرة واحدة.
- أصبح إنشاء QR وربط BreeXe Connect متاحًا مباشرة من مركز الجوال، ومحميًا بصلاحية `mobile.devices.pair` بدل الاعتماد على دور ثابت.
- سياسة الشريحة مغلقة افتراضيًا: أي شريحة شخصية أو مجهولة تُهمل قبل حفظ الرقم أو رفع الحدث. Unifonic يبقى مسار الهاتف المغلق/خارج التغطية.
- أضيفت صلاحيات الجوال الدقيقة مع استثناءات لكل مستخدم، وسجل تدقيق، ومنع خاص لأمر المسح يتطلب مسؤولًا ودخولًا حديثًا واسم الجهاز.
- تطبيق أندرويد أصبح `2.1.0 (6)`: Room + SQLCipher، Keystore، WorkManager، مفاتيح SIM مجهولة، FCM اختياري، تعريف متصل محلي، نتائج ما بعد المكالمة وأوامر اتصال بتأكيد.
...
## آخر تحديث: 2026-07-15 — وضوح الإعداد واختيار الشريحة BreeXe Connect 2.1.2 [Codex]

- أضيف مسار مباشر وآمن لاختيار شريحة العمل من التطبيق بعد الربط؛ لا يستطيع الجهاز اعتماد إلا شريحة فعالة سبق أن رفعها في ملفه، وتبقى جميع الشرائح الأخرى مغلقة افتراضيًا.
- أضيفت بطاقة جاهزية من أربع خطوات داخل التطبيق: ربط الـCRM، الأذونات، اعتماد شريحة العمل، ونجاح أول مزامنة.
- أصبحت شاشة التطبيق تشرح أن تسجيل المكالمة يعني رقمًا ووقتًا ومدةً ونتيجةً فقط، ولا يشمل تسجيل الصوت أو استخدام الميكروفون.
- أضيفت رسالتان قابلتان للتعديل داخل مركز الجوال: داخل الدوام وخارج الدوام، وتستخدمهما المعاينة والتجربة ومسار المكالمة الفعلي.
- أعيد تنظيم مركز الجوال إلى ثلاث خطوات واضحة مع حفظ التبويب في الرابط: الجوال والشريحة، الرسائل والسياسة، وتجربة الإرسال.
- أضيفت حالة واتساب وزر الربط المباشر، مع توضيح أن الرسالة تُرسل من خادم CRM عبر واتساب ويب وليس من جوال الموظف.
- رُفع مخطط SQLite إلى `10310`، وإصدار أندرويد إلى `2.1.2 (8)`.
- دليل التشغيل اليومي: `docs/mobile-onboarding-2.1.2-ar.md`.

## آخر تحديث: 2026-07-16 — حزمة النشر الحية لمركز الجوال 1.4.0 [Codex]

- بُني فرع نشر محدود فوق البناء الحي `db695e9` بدل دمج فرع `main` المتعارض؛ أضيفت إليه فقط منصة الجوال، إصلاح تشغيل Android، واعتماد الشريحة والواجهة الواضحة.
- رُفع إصدار CRM إلى `1.4.0` حتى لا يلتبس مع الإصدار الحي القديم `1.3.9` ذي الاسم المختلف.
- تحقق محليًا ظهور «مركز الجوال»، إنشاء رمز الربط، تبويب اعتماد الشريحة، تحرير رسائل واتساب، وسياسة وتجربة الإرسال.
- التحقق قبل النشر: `382/382` اختبار وحدة، `npm run lint`، `npm run build`، وAndroid `testDebugUnitTest assembleDebug lintDebug`.
- يلزم بعد النشر تسجيل دخول المسؤول، ربط واتساب ويب، إعادة ربط تطبيق `2.1.2`، ثم اعتماد شريحة العمل قبل تجربة مكالمة حقيقية.

## آخر تحديث: 2026-07-16 — إصلاح اعتماد الشريحة وحماية لوحة واتساب 2.1.3 [Codex]

- فصل عرض الشرائح واعتمادها عن أذونات جهات الاتصال والإشعارات؛ يكفي إذن حالة الهاتف لاكتشاف الشرائح.
- ألغيت مطابقة معرفات الشرائح الجزئية، وأضيف ربط دقيق بين شريحة العمل وحساب الاتصال على أجهزة الشريحتين.
- يضيف الـCRM شريحة العمل المعتمدة إلى طلب الاتصال، ولا يعلن التطبيق اكتمال المكالمة بمجرد فتح شاشة الاتصال.
- قيدت حالة بوابة المكالمات التفصيلية بصلاحية الإدارة، وأزيل تجاوز قائمة أرقام الاختبار، وحذفت الأرقام والتسميات من بيانات تدقيق سياسة الرد.
- أصبحت خانة تجربة واتساب فارغة افتراضيًا بدل وضع رقم حقيقي، مع بقاء الإرسال الحقيقي محصورًا بقائمة الخادم المسموحة.
- إصدار أندرويد الجديد `2.1.3 (9)`، ودليل الإصلاح في `docs/android-sim-fix-2.1.3-ar.md`.
...
## 2026-08-05 - BreeXe Pro maintenance customer experience 1.9.7 [Codex]

- Replaced the generic public service form with a branded BreeXe Pro three-step flow: WhatsApp OTP verification, owner-scoped real product search, issue photo/video uploads, admin-controlled available slots, geolocation or allowlisted maps URL, and a personalized tracking page.
- Added a dedicated fail-closed Meta AUTHENTICATION-template channel (`breexe_maintenance_otp_ar`) whose allowlist/production switches are independent from all existing outbound campaigns and reminders. OTP codes and phone numbers are never stored in plaintext or written to canary reports.
- Added admin scheduling controls, product/verification/location/attachment evidence in maintenance request details, and server-side enforcement that rejects UI bypasses for unverified phones, non-catalog products, unavailable slots, unsafe locations, and unclaimed attachments.
...
## 2026-08-15 - GA4 checkout funnel, payment referrals, and TikTok UTM normalization [Codex]

- Reconciled the 80 GA4 transactions from 18 July through 14 August 2026 against the live Golden Pro Salla store: all 80 matched by `transaction_id` and amount, with zero GA-only rows and zero value mismatches. Salla had eight additional orders not present in GA4, totaling SAR 3,980.05.
- Kept business status separate from collected purchase revenue: the 80 matched GA4 transactions include five cancelled orders and four refunded orders, so the GA4 gross purchase total must not be treated as settled net revenue.
- Corrected payment-return attribution in the live Google tag by saving `tamara.co` and `mispay.co` as unwanted referral domains using an `ends with` match. Cross-domain measurement remains empty because the checkout runs on the tagged `goldenksa.store` domain and no second owned domain currently carries the same Google tag.
- Added consent-gated, session-deduplicated `add_shipping_info` and `add_payment_info` events to the Salla Device Mode tracker. The tracker accepts direct and `properties`-wrapped payloads, maps only commerce-safe value/currency/item fields, and never sends customer or raw payment data.
- Canonicalized TikTok attribution stored by the checkout tracker to `utm_source=tiktok` and `utm_medium=paid`. Updated the documented TikTok URL template to include `utm_id=__CAMPAIGN_ID__`, `utm_campaign=__CAMPAIGN_NAME__`, and `utm_content=__AID__` without duplicating Auto-attach parameters.
- Verification passes `32/32` focused analytics tests, `478/478` full unit tests, TypeScript lint, production build, and diff whitespace checks. Production publication and the GA4 TikTok cost connector remain gated on reviewed tracker deployment, TikTok Ads authorization, exact Business/Ad account selection, and applying the single `tiktok / paid` convention to every active destination URL.
