...
- A later `1.9.7` release swap proved that an in-release guard alone was insufficient because the entire application directory, including the guard, could be replaced by an older artifact. The host now also owns `/usr/local/sbin/breexe-stage-caddy-guard` plus systemd path and timer units outside the release directory. They wait for the deployment lock, restore the canonical Stage block, validate it with the active Caddy volumes, recreate only Caddy when necessary, and verify Stage HTTPS every 30 seconds.
